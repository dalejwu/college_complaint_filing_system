...
$category = trim($_POST['category'] ?? $_POST['what_category'] ?? '');
...
if (empty($title) || empty($category) || empty($description) || empty($respondents)) {
    ...
    if (!empty($_FILES['attachments']['name'][0])) {
        foreach ($_FILES['attachments']['name'] as $key => $file_name) {
            if ($_FILES['attachments']['error'][$key] === UPLOAD_ERR_OK) {
                ...
                // Validate file type (images and docs)
                $allowed_types = array(
                    'image/jpeg',
                    'image/png',
                    'image/gif',
    ...
    if (isset($_POST['deleted_attachments']) && is_array($_POST['deleted_attachments'])) {
        $deleted_files = $_POST['deleted_attachments'];
        $existing_attachments = array_diff($existing_attachments, $deleted_files);
    }

    $all_attachments = array_merge($existing_attachments, $uploaded_files);
    $attachments = !empty($all_attachments) ? implode(',', $all_attachments) : '';

    // Update complaint
    $stmt = $conn->prepare("UPDATE complaints SET title = ?, category = ?, description = ?, respondents = ?, respondent_count = ?, attachments = ?, updated_at = NOW() WHERE id = ? AND student_id = ?");
