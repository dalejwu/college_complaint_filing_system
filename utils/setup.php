...
// 2.7: Create admins table if it doesn't exist
try {
    $check = $conn->query("SHOW TABLES LIKE 'admins'");
    if ($check->num_rows == 0) {
        $result = executeQuery($conn, "CREATE TABLE `admins` (
            `id` INT(11) NOT NULL AUTO_INCREMENT,
        ...
        if ($result === true) {
            // Seed default admin
            // Note: Password is plain text as per login.php logic
            executeQuery($conn, "INSERT INTO `admins` (`username`, `password`) VALUES ('admin', 'admin123')");
            $success[] = "Created admins table with default admin user (admin/admin123)";
        } else {
            $warnings[] = "Could not create admins table: " . $result;
        }
    }
} catch (Exception $e) {
    $warnings[] = "Could not create admins table: " . $e->getMessage();
}

// 2.8: Create students, complaints, and activity_logs tables if they don't exist
try {
    // Create students table
    $check = $conn->query("SHOW TABLES LIKE 'students'");
    if ($check->num_rows == 0) {
        $result = executeQuery($conn, "CREATE TABLE `students` (
            `id` INT(11) NOT NULL AUTO_INCREMENT,
            `name` VARCHAR(100) NOT NULL,
        ...
        if ($result === true) {
            ...
            $warnings[] = "Could not create students table: " . $result;
        }
    }

    // Create complaints table
    $check = $conn->query("SHOW TABLES LIKE 'complaints'");
    if ($check->num_rows == 0) {
        $result = executeQuery($conn, "CREATE TABLE `complaints` (
            `id` INT(11) NOT NULL AUTO_INCREMENT,
            `student_id` INT(11) NOT NULL,
            `title` VARCHAR(255) NOT NULL,
            `category` VARCHAR(100) NOT NULL,
            `description` TEXT NOT NULL,
            `respondents` ENUM('Single Person','Multiple People') DEFAULT NULL,
            `respondent_count` INT NULL DEFAULT NULL,
            `status` ENUM('Pending','Approved','Denied','In Progress','Resolved') DEFAULT 'Pending',
            `attachments` TEXT DEFAULT NULL,
            `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
            `deleted_at` TIMESTAMP NULL DEFAULT NULL,
            PRIMARY KEY (`id`),
            KEY `student_id` (`student_id`),
            CONSTRAINT `complaints_student_fk` FOREIGN KEY (`student_id`) REFERENCES `students` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci");

        if ($result === true) {
            $success[] = "Created complaints table";
        } else {
            $warnings[] = "Could not create complaints table: " . $result;
        }
    }
    ...
    if ($check->num_rows == 0) {
        $result = executeQuery($conn, "CREATE TABLE `activity_logs` (
            `id` INT(11) NOT NULL AUTO_INCREMENT,
        ...
        if ($result === true) {
            ...
            $warnings[] = "Could not create activity_logs table: " . $result;
        }
    }
} catch (Exception $e) {
    $warnings[] = "Could not create missing tables: " . $e->getMessage();
}
...
echo "<a href='Login/index.php' style='padding:12px 24px;background:#28a745;color:white;text-decoration:none;border-radius:5px;display:inline-block;'>Go to Login</a>";
echo "</p>";
